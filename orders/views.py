...
from .serializers import (
    OrderDetailSerializer, OrderListSerializer, CheckoutInputSerializer,
    ProducerOrderSerializer, StatusUpdateSerializer
)
...
class CustomerOrderListView(APIView):
    ...
    def get(self, request):
        if request.user.role != 'customer':
            return Response({'error': 'Only customers can view their order list.'}, status=status.HTTP_403_FORBIDDEN)
...
class CustomerOrderDetailView(APIView):
    ...
    def get(self, request, order_number):
        ...
        if request.user.role == 'customer':
            ...
            # Filter to only show producer_orders belonging to this producer
            p_orders = order.producer_orders.filter(producer=request.user)
            if not p_orders.exists():
                return Response(status=status.HTTP_404_NOT_FOUND)
            
            # We still return the order detail but restricted or a specific view
            # Requirement says "filter to only show producer_orders belonging to this producer"
            # I will pass this info in context to the serializer or handle it here.
            serializer = OrderDetailSerializer(order, context={'producer': request.user})
            # Note: The Requirement says "override serializer context or filter queryset"
            # I'll implement a custom logic in Serializer if needed, but for now I'll filter here.
            data = serializer.data
            data['producer_orders'] = [
                po for po in data['producer_orders'] 
                if po['producer_id'] == request.user.id
            ]
...
class ReorderView(APIView):
    ...
    def post(self, request, order_number):
        if request.user.role != 'customer':
            return Response({'error': 'Only customers can reorder.'}, status=status.HTTP_403_FORBIDDEN)
...
class ProducerOrderListView(APIView):
    ...
    def get(self, request):
        if request.user.role != 'producer':
            return Response({'error': 'Only producers can view their sub-orders.'}, status=status.HTTP_403_FORBIDDEN)
...
class ProducerOrderStatusView(APIView):
    ...
    def patch(self, request, producer_order_id):
        if request.user.role != 'producer':
            return Response({'error': 'Only producers can update sub-order status.'}, status=status.HTTP_403_FORBIDDEN)
