        ...
        <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {(result.colour_score > 0 || result.size_score > 0 || result.ripeness_score > 0) && (
            <div style={{ background: "#f8fafc", borderRadius: 14, padding: "18px 20px" }}>
              ...
              <ScoreBar label="Colour Uniformity" score={result.colour_score} grade={bd?.colour?.grade ?? (result.colour_score >= 85 ? "A" : result.colour_score >= 70 ? "B" : "C")} />
              <ScoreBar label="Size Consistency" score={result.size_score} grade={bd?.size?.grade ?? (result.size_score >= 85 ? "A" : result.size_score >= 70 ? "B" : "C")} />
              <ScoreBar label="Ripeness Index" score={result.ripeness_score} grade={bd?.ripeness?.grade ?? (result.ripeness_score >= 80 ? "A" : result.ripeness_score >= 65 ? "B" : "C")} />
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            <label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>Allergens</label>
            <input
              style={inp}
            ...
            <div style={{ fontSize: 12, color: "#6b7280", marginTop: 4 }}>
              Separate multiple allergens with commas. Leave blank if there are no listed allergens.
            </div>
          </div>
                      ...
                      <tr key={p.id} style={{ borderBottom: i < products.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                        <td style={{ padding: "12px 16px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            ...
                            <div>
                              ...
                              {p.allergens && (
                                <div style={{ marginTop: 4, fontSize: 11, color: "#92400e", background: "#fef3c7", display: "inline-block", padding: "2px 7px", borderRadius: 999, fontWeight: 700 }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <div style={{ display: "flex", gap: 6 }}>
                            ...
                            >
                              {assessingId === p.id ? <><span style={{ display: "inline-block", animation: "spin 1s linear infinite" }}>⟳</span> Analysing…</> : <>🤖 Assess Quality</>}
                            </button>
                          </div>
                        </td>
                      </tr>
        ...
        {tab === "ai" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button onClick={loadAiInsights} disabled={insightsLoading} style={{ padding: "8px 16px", borderRadius: 10, border: "1px solid #40916c", background: "#fff", color: "#1b4332", fontWeight: 700, cursor: insightsLoading ? "not-allowed" : "pointer", fontSize: 13 }}>
                {insightsLoading ? "Refreshing…" : "↻ Refresh"}
              </button>
            </div>

            {insightsErr && <div style={{ background: "#fee2e2", color: "#991b1b", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 14 }}>{insightsErr}</div>}
            ...
            {insightsLoading && !aiInsights ? (
              ...
              <>
                <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 24 }}>
                  <StatCard label="Total Assessments" value={aiInsights.grade_statistics.total} sub="all time" accent="#2d6a4f" />
                  <StatCard label="Avg Confidence" value={`${Math.round((aiInsights.grade_statistics.avg_confidence || 0) * 100)}%`} sub="model certainty" accent="#3b82f6" />
                  <StatCard label="Quality Trend" value={TREND_CONFIG[aiInsights.grade_statistics.trend]?.icon + " " + TREND_CONFIG[aiInsights.grade_statistics.trend]?.label} sub="last 20 vs previous 20" accent={TREND_CONFIG[aiInsights.grade_statistics.trend]?.color ?? "#6b7280"} />
                  <StatCard label="Waste Prevented" value={`£${(aiInsights.revenue_impact?.estimated_waste_prevented_gbp || 0).toFixed(0)}`} sub="est. value this week" accent="#8b5cf6" />
                </div>
                  ...
                  <div style={{ background: "#fff", borderRadius: 14, padding: "20px 22px", boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
                    {aiInsights.trend_chart_b64 ? (
                      <>
                        ...
                        <img src={`data:image/png;base64,${aiInsights.trend_chart_b64}`} alt="Quality trend" style={{ width: "100%", borderRadius: 8 }} />
                      </>
                    ) : (
                      <>
                        ...
                        {aiInsights.recent_assessments.map((a, i) => {
                          ...
                          return (
                            <tr key={a.id} style={{ borderBottom: i < aiInsights.recent_assessments.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                              ...
                              <td style={{ padding: "11px 16px", color: "#9ca3af", fontSize: 12 }}>{new Date(a.assessed_at).toLocaleDateString()}</td>
                              <td style={{ padding: "11px 16px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: 24, boxShadow: "0 1px 8px rgba(0,0,0,0.05)", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                ...
                <div style={{ fontSize: 15 }}>
                  {profile?.producer_profile?.store_created_at ? new Date(profile.producer_profile.store_created_at).toLocaleDateString() : "—"}
                </div>
              </div>
              <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: 16, display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  {editMode ? <input style={inp} value={storeName} onChange={(e) => setStoreName(e.target.value)} placeholder="Your store name" /> : <div style={{ fontSize: 15 }}>{storeName || <span style={{ color: "#aaa" }}>—</span>}</div>}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5 }}>Store Description</label>
                  {editMode ? <textarea style={{ ...inp, minHeight: 90, resize: "vertical" }} value={storeDescription} onChange={(e) => setStoreDescription(e.target.value)} placeholder="Tell customers about your farm / business" /> : <div style={{ fontSize: 15, lineHeight: 1.6 }}>{storeDescription || <span style={{ color: "#aaa" }}>—</span>}</div>}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5 }}>Store Contact</label>
                  {editMode ? <textarea style={{ ...inp, minHeight: 80, resize: "vertical" }} value={storeContact} onChange={(e) => setStoreContact(e.target.value)} placeholder="Phone, WhatsApp, Instagram, pickup address…" /> : <div style={{ fontSize: 15, lineHeight: 1.6 }}>{storeContact || <span style={{ color: "#aaa" }}>—</span>}</div>}
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                {!editMode ? (
