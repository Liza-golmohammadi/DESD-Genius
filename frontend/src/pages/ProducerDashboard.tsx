...
// Status transitions (mirrors backend VALID_STATUS_TRANSITIONS)
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `4px solid ${accent ?? "#2d6a4f"}`,
        flex: 1,
        minWidth: 140,
      }}
    >
      <div style={{ fontSize: 12, color: "#888", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.8 }}>
        {label}
      </div>
    ...
    >
      ...
      >
        <h3 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 800 }}>
          {isEdit ? "Edit Inventory" : "Add New Product"}
        </h3>

        {err && (
          <div style={{ background: "#fee2e2", color: "#991b1b", borderRadius: 9, padding: "10px 14px", marginBottom: 16, fontSize: 14 }}>
            {err}
          </div>
        )}
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button
              type="submit"
              disabled={saving}
              style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", background: "#1b4332", color: "#fff", fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", fontSize: 14 }}
            >
            ...
            <button
              type="button"
              onClick={onClose}
              style={{ padding: "10px 20px", borderRadius: 10, border: "1px solid #ddd", background: "#fff", cursor: "pointer", fontSize: 14 }}
            >
              Cancel
            </button>
          </div>
...
export default function ProducerDashboard() {
  ...
  async function saveProfile() {
    ...
    setProfileErr(null);
    try {
      await api.patch("/accounts/producer/me/", { store_name: storeName, description, contact_info: contactInfo });
      await loadProfile();
      setEditMode(false);
  ...
  if (profileLoading) {
    return (
      <div style={{ maxWidth: 900, margin: "60px auto", textAlign: "center", color: "#888" }}>
        Loading dashboard…
      </div>
    );
  }
      ...
      <div style={{ background: "linear-gradient(135deg, #1b4332 0%, #2d6a4f 100%)", padding: "32px 0 28px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            ...
            >
              {(profile?.store_name || profile?.first_name || "P")[0].toUpperCase()}
            </div>
            <div>
              ...
              <h1 style={{ color: "#fff", margin: 0, fontSize: 24, fontWeight: 800 }}>
                {profile?.store_name || `${profile?.first_name} ${profile?.last_name}`.trim() || "Your Store"}
              </h1>
              <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 2 }}>
                {profile?.email} · Member since {profile?.created_at ? new Date(profile.created_at).getFullYear() : "—"}
              </div>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 24px" }}>
                  ...
                  <thead>
                    <tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                      {["Product", "Category", "Price", "Stock", "Status", "Actions"].map((h) => (
                        <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontWeight: 700, color: "#374151", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {products.map((p, i) => (
                      <tr key={p.id} style={{ borderBottom: i < products.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                        <td style={{ padding: "12px 16px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            {p.image_url ? (
                              ...
                              <div style={{ width: 40, height: 40, borderRadius: 8, background: "#e8f5e9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
                                🥦
                              </div>
                            )}
                            <div>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ fontWeight: 600, color: p.is_low_stock ? "#d97706" : "#111" }}>
                            {p.stock_quantity}
                          </span>
                          {p.is_low_stock && (
                            <span style={{ marginLeft: 6, fontSize: 11, color: "#d97706", fontWeight: 600 }}>⚠ Low</span>
                          )}
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          <span
                            style={{
                              ...
                              fontSize: 12,
                              fontWeight: 700,
                              background: p.is_available ? "#d1fae5" : "#fee2e2",
                          ...
                          {p.organic_certified && (
                            <span style={{ marginLeft: 6, fontSize: 11, color: "#2d6a4f", fontWeight: 700 }}>🌿 Organic</span>
                          )}
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          <button
                            onClick={() => { setEditProduct(p); setShowModal(true); }}
                            style={{ padding: "5px 12px", borderRadius: 8, border: "1px solid #ddd", background: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600, color: "#374151" }}
                          >
                            Edit Stock
                          </button>
                        </td>
        ...
        {tab === "profile" && (
          <div style={{ maxWidth: 600 }}>
            ...
            {profileMsg && (
              <div style={{ background: "#d1fae5", color: "#065f46", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 14 }}>
                {profileMsg}
              </div>
            )}
            {profileErr && (
              <div style={{ background: "#fee2e2", color: "#991b1b", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 14 }}>
                {profileErr}
              </div>
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: 24, boxShadow: "0 1px 8px rgba(0,0,0,0.05)", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: 16, display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  {editMode ? (
                    ...
                    <div style={{ fontSize: 15 }}>{profile?.store_name || <span style={{ color: "#aaa" }}>—</span>}</div>
                  )}
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5 }}>Description</label>
                  {editMode ? (
                    <textarea style={{ ...inp, minHeight: 90, resize: "vertical" }} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Tell customers about your farm / business" />
                  ) : (
                    <div style={{ fontSize: 15, lineHeight: 1.6 }}>{profile?.description || <span style={{ color: "#aaa" }}>—</span>}</div>
                  )}
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5 }}>Contact Info</label>
                  {editMode ? (
                    <textarea style={{ ...inp, minHeight: 80, resize: "vertical" }} value={contactInfo} onChange={(e) => setContactInfo(e.target.value)} placeholder="Phone, WhatsApp, Instagram, pickup address…" />
                  ) : (
                    <div style={{ fontSize: 15, lineHeight: 1.6 }}>{contactInfo || <span style={{ color: "#aaa" }}>—</span>}</div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                {!editMode ? (
                  <button
                    onClick={() => { setProfileMsg(null); setProfileErr(null); setEditMode(true); }}
                    style={{ padding: "10px 18px", borderRadius: 10, border: "1px solid #ddd", background: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 14 }}
                  >
                    Edit Profile
                  </button>
                ) : (
                  <>
                    <button
                      ...
                      style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: "#1b4332", color: "#fff", fontWeight: 700, cursor: profileSaving ? "not-allowed" : "pointer", fontSize: 14 }}
                    >
                      {profileSaving ? "Saving…" : "Save Changes"}
                    </button>
                    <button
                      onClick={() => { setEditMode(false); setStoreName(profile?.store_name ?? ""); setDescription(profile?.description ?? ""); setContactInfo(profile?.contact_info ?? ""); }}
